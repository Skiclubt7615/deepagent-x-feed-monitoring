...
function getWsUrl() {
  const candidates = [
    ...
    resolve(homedir(), 'Library/Application Support/Arc/User Data/DevToolsActivePort'),
    resolve(homedir(), 'Library/Application Support/Chromium/DevToolsActivePort'),
  ];
  const portFile = candidates.find(f => { try { readFileSync(f); return true; } catch { return false; } });
  if (!portFile) throw new Error('No browser DevToolsActivePort found. Enable remote debugging in Chrome or Arc.');
  const lines = readFileSync(portFile, 'utf8').trim().split('\n');
  // Use localhost instead of 127.0.0.1 — Chrome rejects IP-based Host headers.
